...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    ...
    foreach ($fileContent as $number) {
        ...
        if (preg_match('/^\+?\d+$/', $number)) {
            $command = "asterisk -rx \"database del $blacklist $number \"";
    ...
    if (!empty($deletedNumbers)) {
        $response = array(
            'success' => true,
            'message' => count($deletedNumbers) . ' numbers were deleted from blacklist: ' . $blacklist,
            'failed' => $failedNumbers
        );
    } else {
        $response = array(
            'success' => false,
            'message' => 'No valid numbers were deleted to the blacklist.',
            'failed' => $failedNumbers
        );
    }
